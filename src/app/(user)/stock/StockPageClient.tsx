...
export default function StockPageClient() {
  ...
  useEffect(() => {
    const fetchLastImport = async () => {
      try {
        const meta = await stockService.getLastImportMeta();
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <ComponentCard 
          ...
          action={
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-end gap-0.6 text-sm text-gray-600 dark:text-gray-400">
        ...
        >
          <BasicTableOne1 key={refreshKeyRef.current} onExportReady={handleExportReady} />
        </ComponentCard>
      </div>
    </div>
  );
