...
import { complaintsService, IComplaintTypeStat } from "@/services/complaints.service";
import ComplaintsTypeDonutChart from "../charts/ComplaintsTypeDonutChart";

export default function ComplaintsTypeStatsTable() {
  const [stats, setStats] = useState<IComplaintTypeStat[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<'count' | null>(null);
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const data = await complaintsService.getTypeStats();
      ...
      <div className="w-full lg:w-[40%] overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03] p-6">
        <div className="mb-4">
          <h3 className="text-base font-medium text-gray-800 dark:text-white/90 mb-4">
            Распределение по типам претензий
          </h3>
        </div>
        {loading ? (
          ...
          <ComplaintsTypeDonutChart data={stats} />
        )}
      </div>
    </div>
  );
