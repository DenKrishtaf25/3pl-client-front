...
export interface IOrdersChartDataPoint {
  date: string; // "YYYY-MM-DD"
  quantityByPlannedDate: number; // По плановой дате
  quantityByActualDate: number; // По фактической дате
}

interface LineChartOrdersProps {
  data: IOrdersChartDataPoint[];
  onZoom?: (dateFrom: string, dateTo: string) => void;
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
export default function LineChartOrders({ data, onZoom: _onZoom }: LineChartOrdersProps) {
  ...
  const series = useMemo(() => {
    if (!data || data.length === 0) {
      return [
        { name: "По плановой дате", data: [] },
        { name: "По фактической дате", data: [] },
      ];
    }

    const plannedData = data.map(item => ({
      x: new Date(item.date).getTime(),
      y: item.quantityByPlannedDate
    }));
    const actualData = data.map(item => ({
      x: new Date(item.date).getTime(),
      y: item.quantityByActualDate
    }));

    // Ограничиваем количество точек до 150
    const maxPoints = 150;
    const limitedPlannedData = limitDataPoints(plannedData, maxPoints);
    const limitedActualData = limitDataPoints(actualData, maxPoints);

    return [
      {
        name: "По плановой дате",
        data: limitedPlannedData,
      },
      {
        name: "По фактической дате",
        data: limitedActualData,
      },
    ];
  ...
  const options: ApexOptions = useMemo(() => ({
    ...
    colors: ["#8A1817", "#2D995B"], // Define line colors
    chart: {
  ...
  return (
    <div className="max-w-full overflow-x-auto custom-scrollbar">
      <div id="chartOrders" className="min-w-[1000px]">
        <ReactApexChart
          options={options}
          series={series}
          type="area"
