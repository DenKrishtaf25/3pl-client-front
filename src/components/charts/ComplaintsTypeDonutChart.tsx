...
import { IComplaintTypeStat } from "@/services/complaints.service";
...
interface ComplaintsTypeDonutChartProps {
  data: IComplaintTypeStat[];
}
...
export default function ComplaintsTypeDonutChart({ data }: ComplaintsTypeDonutChartProps) {
  ...
  // Генерируем контрастные цвета для типов
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (centerLabel) {
        // Показываем тип и значение при наведении
        chart.updateOptions({
          plotOptions: {
            pie: {
              donut: {
